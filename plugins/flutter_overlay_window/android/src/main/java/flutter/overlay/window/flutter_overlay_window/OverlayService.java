...
public class OverlayService extends Service implements View.OnTouchListener {
    ...
    private Rect getSafeArea() {
        ...
        int right = szWindow.x;
        int bottom = szWindow.y - navigationBarHeightPx();
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
            ...
            if (wm != null) {
                ...
                android.graphics.Insets systemInsets = insets.getInsetsIgnoringVisibility(
                        WindowInsets.Type.systemBars() | WindowInsets.Type.displayCutout()
                );
                Rect bounds = metrics.getBounds();
                left = systemInsets.left;
                top = systemInsets.top;
                right = bounds.width() - systemInsets.right;
                bottom = bounds.height() - systemInsets.bottom;
            }
        }
    ...
    private void saveOverlayPosition(int x, int y) {
        SharedPreferences prefs = getApplicationContext().getSharedPreferences(
                OverlayPluginConstants.OVERLAY_ENABLED, Context.MODE_PRIVATE
        );
        prefs.edit()
                .putFloat(OverlayPluginConstants.OVERLAY_X, (float) pxToDp(x))
                .putFloat(OverlayPluginConstants.OVERLAY_Y, (float) pxToDp(y))
                .apply();
    }
    ...
    private void launchMainActivity(String payload) {
        ...
        if (intent != null) {
            ...
            if (payload != null) {
                intent.putExtra(OverlayPluginConstants.FIELD_PAYLOAD, payload);
            }
        ...
        if (windowManager != null && WindowSetup.enableDrag) {
            ...
            switch (event.getAction()) {
                ...
                case MotionEvent.ACTION_MOVE:
                    ...
                    int petWidth = flutterView.getWidth();
                    int petHeight = flutterView.getHeight();
                    params.x = clamp(xx, safe.left - petWidth / 2, safe.right - petWidth / 2);
                    params.y = clamp(yy, safe.top, safe.bottom - petHeight);
    ...
    private class TrayAnimationTimerTask extends TimerTask {
        ...
        public TrayAnimationTimerTask() {
            ...
            Rect safe = getSafeArea();
            int petWidth = flutterView.getWidth();
            mDestY = clamp(lastYPosition, safe.top, safe.bottom - flutterView.getHeight());
            switch (WindowSetup.positionGravity) {
                case "auto":
                    mDestX = params.x <= safe.centerX() - petWidth / 2
                            ? safe.left
                            : safe.right - petWidth;
                    return;
                case "left":
